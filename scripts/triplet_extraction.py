...
def extract_triplets_with_gpt(report_text, client):
    """
...
def process_json(json_path, openai_key):
    """
    ...
    for entry in tqdm(train_data):
        ...
        for section_name, section_text in text_sections.items():
            if section_text.strip():
                try:
                    gpt_output = extract_triplets_with_gpt(section_text, client)
                    i = gpt_output.find("{")
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Extract triplets from CT findings")
    p.add_argument("--json_path", required=True)
    p.add_argument("--openai_key", required=True,
                   help="Your OpenAI API key")
    args = p.parse_args()

    process_json(json_path=args.json_path, openai_key=args.openai_key)