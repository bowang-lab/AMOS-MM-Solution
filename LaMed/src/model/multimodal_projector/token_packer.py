...
class TokenPacker(nn.Module):
    def __init__(
        ...
        super().__init__()
        if any([rg % sf != 0 for rg, sf in zip(raw_grid, scale_factor)]):
            raise ValueError("Each scale_factor must be divisible by corresponding grid size")
        self.raw_grid = raw_grid
        self.grid_size = tuple(rg // sf for rg, sf in zip(raw_grid, scale_factor))
        self.num_queries = self.grid_size[0] * self.grid_size[1] * self.grid_size[2]
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.scale_factor = scale_factor
        self.ratios = [k/sum(self.raw_grid) for k in self.raw_grid]
        self.q_proj_1 = nn.Linear(kv_dim, embed_dim, bias=False)

        k_modules = [nn.Linear(3072, 1024)]
        for _ in range(1, 2):
            k_modules.append(nn.GELU())
            k_modules.append(nn.Linear(1024, 1024))
        self.k_proj_1 = nn.Sequential(*k_modules)

        v_modules = [nn.Linear(3072, 1024)]
        for _ in range(1, 2):
    ...
    def divide_feature(self, x, kernel_size, token_num, N, c):
        new_d, new_h, new_w = self.grid_size

        x = x.reshape(new_d, kernel_size[0], new_h, kernel_size[1], new_w, kernel_size[2], N, c)
        x = x.permute(1, 3, 5, 0, 2, 4, 6, 7).reshape(kernel_size[0] * kernel_size[1] * kernel_size[2], -1, c)

        return x

    def forward(self, x, attn_mask=None):
        x_multi = x[1]  # multi-level
        ...
        q = F.interpolate(x.reshape(x.shape[0], d, h, w, -1).float().permute(0, 4, 1, 2, 3),
                          size=(new_d, new_h, new_w), mode='trilinear').permute(0, 2, 3, 4, 1)
        q = q.reshape(q.shape[0], -1, q.shape[-1]).to(x.dtype)

        query = self.ln_q_1(self.q_proj_1(q)).permute(1, 0, 2)

        reshape_query = self.divide_feature(query, (1, 1, 1), self.num_queries, N, c)
